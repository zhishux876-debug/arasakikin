...
- 対象: `output/yaaac-stamp-dial-craft/index.html`（単一HTML・画像ファイルなし）
...
| # | 区画 | 中身 |
| ---: | :--- | :--- |
| 1 | `.topmark` | 主催者名「アラサー基金」（字間 .3em）／`山梨`（I-14） |
| 2 | `.hero` | **2カラム**。左 `.hero-main`（名称→キャッチ→罫線→説明文）／右 `.rail` |
| 3 | `.rail` | 頭字語（縦読み・罫線で区切る）→ 募集期間 → 採択者決定 → CTA |
| 4 | `#about` | 導入文（小さく組む） |
| 5 | `#why` | 3章（思い / なんで３０日？ / なんで３０万？）。既定は「なんで３０日？」 |
| 6 | `#guidelines` | 募集要項（対象・資格・審査基準・流れ・方法） |
| 7 | `footer` | Follow US / 財団のSNS / 問い合わせ先 / 主催表記「アラサー基金（山梨）」 |
...
| I-14 | **主催の表記は「アラサー基金」。** 「河原部社」「特定非営利活動法人河原部社」を出さない | 2026-08-28「団体名が河原部社になっているけどアラサー基金に名前変えておいて」 |
