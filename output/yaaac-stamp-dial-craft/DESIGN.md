...
| 部品 | 規則 |
| :--- | :--- |
| **主催ロゴ（`.logo`）** | `<a>` で `https://yamanashifund.org/` へ送る（別タブ・`rel="noopener noreferrer"`・確定文言の外に視覚非表示の注記＝I-10）。中身は `<picture>` で webp 2段（200/400）＋ jpg フォールバック。**白い札**（`#fff`・角丸 `10px`・墨 `1.5px` の細罫）の上に置き、`width`/`height` を必ず書く（レイアウトシフト防止）。天は幅 `132px`（実寸 ≒49px で操作領域 44px を満たす）、フッターは `176px`。天のロゴだけ `loading="eager"` ＋ `fetchpriority="high"`。比 3.74 を崩さない |
