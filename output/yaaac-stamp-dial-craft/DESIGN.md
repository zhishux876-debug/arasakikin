...
| 説明文 | `clamp(14.5px,3.3vw,17px)` / `--ink-soft` / 幅 24em |
...
| # | 区画 | 中身 |
| ---: | :--- | :--- |
| 1 | `.topmark` | 主催ロゴ「山梨もしも財団」（白い札の上・I-14）。**文字の主催表記は置かない** |
| 2 | `.hero` | **2カラム**。左 `.hero-main`（名称→キャッチ→罫線→説明文）／右 `.rail` |
...
| 部品 | 規則 |
| :--- | :--- |
| **主催ロゴ（`.logo`）** | `<picture>` で webp 2段（200/400）＋ jpg フォールバック。**白い札**（`#fff`・角丸 `10px`・墨 `1.5px` の細罫）の上に置き、`width`/`height` を必ず書く（レイアウトシフト防止）。天は幅 `132px`、フッターは `176px`。天のロゴだけ `loading="eager"` ＋ `fetchpriority="high"`。比 3.74 を崩さない |
...
| I-14 | **主催は支給されたロゴ「山梨もしも財団」で表す**（天とフッターの2か所）。「河原部社」を出さない。ロゴは白い札の上に置く（クラフト紙に直に載せると緑寄りの部分が地と同化する） | 2026-08-28「団体名が河原部社になっているけどアラサー基金に名前変えておいて」→ 同日「アラサー基金って文言でかいてあるところにこの画像を差し替えて。メインページとページ下部のところね」 |
...
# ---- I-14: 主催は支給されたロゴで表す（文字の団体名を紙面に置かない） ----
